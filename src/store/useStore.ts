...
export const RoutineRunner = ({ programId, onClose }: RoutineRunnerProps) => {
  ...
  const handleRatingSelection = (ratingValue: number) => {
    ...
    if (isLastExercise && isUnilateralFinished) {
      if (program) {
        // HIER ERZWINGEN WIR DIE ID-GENERIERUNG DIREKT BEIM SENDEN
        addHistoryEntry({
  ...
  if (phase === 'FINISHED' || !currentProgramData || !currentExerciseData || !program) {
    return (
      <div className="flex flex-col items-center justify-center p-6 h-[100dvh] bg-gradient-to-b from-[#0d0f12] via-[#08090a] to-[#030405] text-slate-100 gap-6 select-none fixed inset-0 z-50">
  ...
  return (
    <div className="flex flex-col items-center justify-between p-6 text-slate-100 bg-gradient-to-b from-[#0d0f12] via-[#08090a] to-[#030405] h-[100dvh] fixed inset-0 box-border overflow-hidden select-none max-w-lg mx-auto z-50">
